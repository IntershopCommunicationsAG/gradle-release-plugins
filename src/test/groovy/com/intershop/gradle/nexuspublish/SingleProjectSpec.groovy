...
class SingleProjectSpec extends AbstractIntegrationSpec {
    ...
    def 'test snapshot publishing without staging'() {
        given:
        String urlStr = server.url('/').toString()
        List<String> upLoadList = []
        Map<String,String> responses = [:]
        ...
        when:
        def result = getPreparedGradleRunner()
                .withArguments('publish', '--stacktrace', '-i', "-DRUNONCI=true", '-PjiraFieldName=Labels', "-DNEXUSBASEURL=${urlStr}nexus/", '-DNEXUSUSERNAME=admin', '-DNEXUSUSERPASSWD=admin123', "-DJIRABASEURL=${urlStr}", '-DJIRAUSERNAME=admin', '-DJIRAUSERPASSWD=admin123')
                .build()
